...
- **Cloudflare Workers** — a fetch-event compatibility layer (`src/worker_runtime.c`) bridges a Worker's request/response model to the library's request/response types (**not** to the router — `worker_set_router()` is accepted but never dispatched), with pure-C APIs shaped like the KV, R2, D1 and Queues bindings. **In every build — native, test, and WASM — these are backed by in-memory simulations**, not by Cloudflare's services: reaching the real bindings needs a JS glue layer, and none ships in this repo (`examples/worker.js` accepts `env` but never passes it into WASM, and no `wrangler.toml` ships). The simulations have fixed capacities, and `worker_d1_batch()` is not atomic the way Cloudflare's `env.DB.batch()` is. Details in [Cloudflare Workers API](WORKER_API.md).
